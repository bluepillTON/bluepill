...
function App() {
  ...
  const [chatMessages, setChatMessages] = useState<Record<string, {role: 'user' | 'demon' | 'heaven', text: string}[]>>({});
  ...
  const generateResponse = (agent: Agent, msg: string) => {
  ...
  return (
    <div className="app-container">
      ...
      <div className="bottom-nav">
        ...
        <div onClick={() => setActiveTab('stake')} className={`nav-item ${activeTab === 'stake' ? 'active' : ''}`}>💰</div>
        <div onClick={() => setActiveTab('predictions')} className={`nav-item ${activeTab === 'predictions' ? 'active' : ''}`}>📈</div>
      </div>
